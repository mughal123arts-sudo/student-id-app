...
fun AppNavHost(viewModel: StudentViewModel) {
    ...
    val startDestination = remember { if (AppPreferences.isSetupComplete(context)) "login" else "setup" }
    ...
    NavHost(navController = navController, startDestination = startDestination) {
        ...
        composable(
            ...
            LaunchedEffect(studentId) {
                if (studentId != -1) {
                    existingStudent = viewModel.allStudents.value.find { it.id == studentId }
                    ready = true
                }
            }
        ...
        composable(
            ...
            LaunchedEffect(studentId) {
                student = viewModel.allStudents.value.find { it.id == studentId }
            }
